                            ...
                            <div class="flex flex-row md:flex-col my-2">
                                <div class="w-full mx-4">
                                    ...
                                    <select name="user_id" 
                                        ...
                                        <option value="{{ old('user_id', $data->user_id) }}">{{ old('user_id', $data->user_id) }}
                                        </option>
                                    </select>
                                </div>
                                <div class="w-full mx-4">
                                    <label for="first_name"
                                        class="block mb-2  text-sm font-medium text-gray-900 dark:text-white">ID Pembina</label>
                                    <select name="pembina_id" id="pembina"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">

                                        <option value="{{ old('pembina_id', $data->pembina_id) }}">{{ old('pembina_id', $data->pembina_id) }}
                                        </option>
                                        @foreach ($pembina as $item)
                                            <option value="{{ $item->id }}" data-pembina="{{ $item->admin->nama }}" data-status="{{ $item->status }}" data-bidang-kerja="{{ $item->bidang_kerja }}">
                                                {{ $item->id }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="flex flex-row md:flex-col my-2">
                                <div class="w-full mx-4">
                                    <label for="first_name"
                                        ...
                                        <input type="text" id="input_status"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                        placeholder="2986 Kaden Tunnel Suite 373 Kuphalville, KS 28321" required
                                        " readonly>
                                </div>
                                <div class="w-full mx-4">
                                    <label for="first_name"
                                        class="block mb-2  text-sm font-medium text-gray-900 dark:text-white">Nama Pembina</label>
